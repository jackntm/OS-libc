...
int __free(struct pcb_t *caller, int vmaid, int rgid)
{
  ...
  //enlist_vm_freerg_list();
    ...
    // Xoá thông tin khỏi bảng symbol
    rg->rg_start = 0;
    rg->rg_end = 0;
    return 0;
}
...
int get_free_vmrg_area(struct pcb_t *caller, int vmaid, int size, struct vm_rg_struct *newrg)
{
  ...
  while (rgit != NULL)
  {
      if (rgit->rg_end - rgit->rg_start + 1 >= size)
      {
          ...
          /* Cập nhật lại vùng free sau khi trừ ra phần cấp phát */
          rgit->rg_start += size;

          /* Nếu vùng free bị cấp phát hết thì xoá khỏi danh sách */
          if (rgit->rg_start > rgit->rg_end)
          {
              // Xoá rgit khỏi danh sách liên kết
