 ...
 */
int vmap_page_range(struct pcb_t *caller,           // process call   	Process cần ánh xạ RAM
                    int addr,                       // start address which is aligned to pagesz  	Địa chỉ ảo bắt đầu (phải aligned với PAGING_PAGESZ)
...
{                                                   // no guarantee all given pages are mapped
 ...
 ret_rg->rg_end = addr;
  ...
  for (pgit = 0; pgit < pgnum; pgit++) {
    if (fpit == NULL) {
      // Hết frame vật lý để map, trả lỗi
      ret_val = -1;
      break;
    }
  ...
  */
   ...
   */
    ...
    { // TODO: ERROR CODE of obtaining somes but not enough frames
      struct framephy_struct *iter = newfp_str;
			while (iter != NULL) {
				MEMPHY_put_freefp(caller->mram, iter->fpn);
				struct framephy_struct *next = iter->fp_next;
				free(iter);
				iter = next;
			}
			newfp_str = NULL;
